...
interface ToastItem { id: number; mensagem: string; tipo: ToastTipo; }
interface ToastContextValue { notificar: (mensagem: string, tipo?: ToastTipo) => void; }
...
const BORDAS: Record<ToastTipo, string> = {
  sucesso: 'border-primary/40', erro: 'border-red-500/40', info: 'border-gold/40',
};
  ...
  return (
    <ToastContext.Provider value={{ notificar }}>
      ...
      <div className="fixed top-4 right-4 z-[100] flex flex-col gap-2 w-[calc(100%-2rem)] max-w-sm">
        {toasts.map((t) => (
          <div key={t.id} className={`animate-slideUp flex items-start gap-2 bg-bg-card border ${BORDAS[t.tipo]} rounded-xl px-4 py-3 shadow-lg shadow-black/30`}>
            {ICONES[t.tipo]}
            <p className="text-sm flex-1 leading-snug">{t.mensagem}</p>
            <button onClick={() => remover(t.id)} className="text-gray-500 hover:text-gray-300"><X size={16} /></button>
          </div>
        ))}
      </div>
