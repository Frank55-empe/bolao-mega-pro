...
export default function AdminParticipantes() {
  ...
  const filtradas = useMemo(() => apostas.filter((a) => {
  ...
  async function alterarStatus(id: string, novoStatus: string) {
    ...
    } else notificar(resp.error || 'Não foi possível atualizar.', 'erro');
  }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="flex justify-between items-center mb-6 flex-wrap gap-3">
        ...
        <button onClick={exportarCsv} className="flex items-center gap-2 bg-white/5 hover:bg-white/10 rounded-xl px-4 py-2 text-sm focus-ring"><Download size={16} /> Exportar CSV</button>
      </div>
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <div className="relative flex-1">
          <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" />
          <input className="input pl-9" placeholder="Buscar por nome, protocolo, cidade..." value={busca} onChange={(e) => { setBusca(e.target.value); setPagina(1); }} />
        </div>
        <select className="input sm:w-56" value={filtroStatus} onChange={(e) => { setFiltroStatus(e.target.value); setPagina(1); }}>
          ...
          <div className="overflow-x-auto rounded-2xl border border-white/5">
            <table className="w-full text-sm">
              <thead className="bg-bg-card text-gray-400 text-left">
                <tr><th className="p-3">Protocolo</th><th className="p-3">Números</th><th className="p-3">Valor</th><th className="p-3">Status</th><th className="p-3">Data</th></tr>
              </thead>
              <tbody>
                {pagina_atual.map((a) => (
                  <tr key={a.id} className="border-t border-white/5 hover:bg-white/[0.02]">
                    ...
                    <td className="p-3">
                      <select className="bg-bg-soft border border-white/10 rounded-lg text-xs px-2 py-1 focus-ring" value={a.status} onChange={(e) => alterarStatus(a.id, e.target.value)}>
                        {STATUS_FILTROS.filter((s) => s !== 'TODOS').map((s) => (<option key={s} value={s}>{s}</option>))}
                      </select>
                    </td>
                    <td className="p-3 text-gray-400">{a.data}</td>
                  </tr>
                ))}
                {pagina_atual.length === 0 && (<tr><td colSpan={5} className="p-6 text-center text-gray-500">Nada encontrado.</td></tr>)}
              </tbody>
            </table>
          </div>
          {totalPaginas > 1 && (
            <div className="flex justify-center items-center gap-3 mt-4 text-sm">
              <button disabled={pagina === 1} onClick={() => setPagina((p) => p - 1)} className="p-2 rounded-lg hover:bg-white/5 disabled:opacity-30 focus-ring"><ChevronLeft size={16} /></button>
              <span className="text-gray-400">Página {pagina} de {totalPaginas}</span>
              <button disabled={pagina === totalPaginas} onClick={() => setPagina((p) => p + 1)} className="p-2 rounded-lg hover:bg-white/5 disabled:opacity-30 focus-ring"><ChevronRight size={16} /></button>
            </div>
          )}
        </>
      )}
