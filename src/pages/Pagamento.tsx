...
export default function Pagamento() {
  ...
  useEffect(() => {
    if (!participante || numerosEscolhidos.length === 0) { navigate('/cadastro'); return; }
    setProtocoloLocal(gerarProtocolo());
  }, []);

  const pixPayload = gerarPayloadPix({ chave: CHAVE_PIX, nomeRecebedor: NOME_RECEBEDOR, cidade: CIDADE_RECEBEDOR, valor: valorAposta, txid: protocoloLocal || undefined });
  ...
  async function jaPaguei() {
    ...
    const respParticipante = await api.participantes.criar({
      nome: participante.nome, nomeExibicao: participante.nomeExibicao, whatsapp: participante.whatsapp,
      cidade: participante.cidade, estado: participante.estado, cpf: participante.cpf ?? '',
    });
    ...
    const respAposta = await api.apostas.criar({
      idParticipante: participanteId, concursoId: concurso.id, numeros: numerosEscolhidos,
      quantidade: numerosEscolhidos.length, valor: valorAposta, protocolo: protocoloLocal, status: 'AGUARDANDO CONFERENCIA',
    });
    if (!respAposta.ok) {
      setEnviando(false);
    ...
    if (idApostaCriada) await api.pagamentos.confirmar(idApostaCriada);
    setEnviando(false);
  ...
  function abrirWhatsApp() {
    if (!participante || !concurso) return;
    const msg = montarMensagemWhatsApp({ nome: participante.nomeExibicao, protocolo: protocoloLocal, numeros: numerosEscolhidos, valor: valorAposta, concurso: concurso.nome });
  ...
  if (pago) {
    return (
      <div className="max-w-md mx-auto px-4 py-16 text-center">
        <span className="w-16 h-16 rounded-full bg-primary/15 flex items-center justify-center mx-auto mb-4 animate-popIn"><PartyPopper className="text-primary" size={28} /></span>
        ...
        <p className="text-gray-300 text-sm mb-6">Assim que confirmarmos o pagamento, seu status muda para <strong>PAGO</strong>. Você pode acompanhar tudo em "Meus Jogos".</p>
        <div className="flex flex-col gap-3">
          <button onClick={abrirWhatsApp} className="bg-primary hover:bg-primary-dark text-white font-semibold rounded-xl py-3 focus-ring">Enviar comprovante pelo WhatsApp</button>
          <button onClick={() => { limparFluxo(); navigate('/consulta'); }} className="border border-white/10 rounded-xl py-3 focus-ring">Ver meus jogos</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-md mx-auto px-4 py-8">
      ...
      <div className="bg-bg-card rounded-2xl p-5 border border-white/5 mb-5 flex flex-col items-center">
        ...
        <button onClick={copiarChave} className="w-full flex items-center justify-center gap-2 bg-white/5 hover:bg-white/10 rounded-xl py-3 text-sm focus-ring">
          {copiado ? <CheckCheck size={16} className="text-primary" /> : <Copy size={16} />}
          {copiado ? 'Copiado!' : 'Copiar chave PIX'}
        </button>
      </div>
      <button onClick={jaPaguei} disabled={enviando} className="w-full bg-primary hover:bg-primary-dark disabled:opacity-60 text-white font-semibold rounded-xl py-3 focus-ring">
...
function Linha({ label, valor, destaque = false }: { label: string; valor: string; destaque?: boolean }) {
  return (<div className="flex justify-between"><span className="text-gray-400">{label}</span><span className={destaque ? 'font-bold text-primary-light' : 'font-medium'}>{valor}</span></div>);
}
