  ...
  useEffect(() => {
    api.concursos.ativo().then((resp) => {
      if (resp.ok && resp.data) {
        ...
        notificar(resp.error || 'Não foi possível falar com o servidor.', 'erro');
        console.error('[concursos/ativo] falhou:', resp);
      } else {
        console.warn('[concursos/ativo] sem concurso ativo.');
      }
      setCarregando(false);
    });
  }, []);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8">
      <section className="bg-gradient-to-br from-bg-card to-bg-soft rounded-3xl p-6 sm:p-10 border border-white/5 relative overflow-hidden">
        ...
        {carregando ? (
          <div className="space-y-4 relative"><SkeletonCard /></div>
        ) : concurso ? (
          <div className="relative">
            <p className="text-gold font-semibold text-sm tracking-wide uppercase mb-1">{concurso.nome} · Concurso {concurso.numero}</p>
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <Link to="/cadastro" className="bg-primary hover:bg-primary-dark transition-colors text-white font-semibold rounded-xl px-6 py-3 text-center focus-ring">Quero participar</Link>
              <Link to="/consulta" className="bg-white/5 hover:bg-white/10 transition-colors rounded-xl px-6 py-3 text-center flex items-center justify-center gap-2 focus-ring"><ListChecks size={18} /> Ver meus jogos</Link>
              <Link to="/regulamento" className="text-gray-400 hover:text-gray-200 rounded-xl px-6 py-3 text-center flex items-center justify-center gap-2 focus-ring"><ScrollText size={18} /> Regulamento</Link>
            </div>
          </div>
...
function InfoStat({ icone, label, valor }: { icone: React.ReactNode; label: string; valor: string }) {
  return (<div className="bg-black/20 rounded-xl p-3"><div className="flex items-center gap-1.5 text-gray-400 text-xs mb-1">{icone}{label}</div><p className="font-bold text-sm sm:text-base">{valor}</p></div>);
}
